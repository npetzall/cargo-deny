...
use crate::sarif::model::{Message, ArtifactContent};
    ...
    fn process(
        ...
        if let Some(first_secondary) = secondary_labels.first() {
            if let Ok(secondary_loc) = files.sarif_location(first_secondary) {
                if let Some(ArtifactContent::Text { ref text }) = secondary_loc.physical_location.region.snippet {
                    if !md.is_empty() {
                        md.push_str("\n\n");
                    }
                    md.push_str(text);
        ...
        if !primary_labels.is_empty() {
            ...
            for label in &primary_labels {
                ...
                let license_name = files
                    ...
                    .and_then(|snippet| match snippet {
